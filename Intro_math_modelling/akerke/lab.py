...
while iter<100:
    ...
    for i in range(1,N):
        for j in range(1,N):
            ...
            jacobi[i][j]=(jacobi_old[i+1][j]+jacobi_old[i-1][j]+jacobi_old[i][j+1]+jacobi_old[i][j-1])/4
    u=deepcopy(new)
    jacobi_old=deepcopy(jacobi)
    if iter%10==0:
        ...
        cp1 = ax1.contourf(x,y,transpose(jacobi_old))
        fig1.colorbar(cp1) # Add a colorbar to a plot
        ax1.set_title('Jacobi method: t = '+str(iter*dt) + " N = "+str(N))
        ax1.set_xlabel('x ')
        ax1.set_ylabel('y ')
        plt.show()  
